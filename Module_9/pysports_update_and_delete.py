...
try:
    ...
    add_player = ("INSERT INTO player(first_name, last_name, team_id)"
                 "VALUES(%s, %s, %s)")
    player_data = ("Williams", "Dahn", 1)
    cursor.execute(add_player, player_data)
    db.commit()
    print()
    display_players(cursor, "DISPLAYING PLAYERS AFTER INSERT")
    print()
    update_player = ("UPDATE player SET team_id = 2, first_name = 'John', last_name = 'Willie' WHERE first_name = 'Williams'")
    cursor.execute(update_player)
    display_players(cursor, "DISPLAYING PLAYERS AFTER UPDATE")
    print()
    delete_player = ("DELETE FROM player WHERE first_name = 'John'")
